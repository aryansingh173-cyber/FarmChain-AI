...
export default function PublicQRVerificationPage() {
  ...
  const batch = getBatchById(batchId) || getBatchById('FC-2026-APL-8821');
  ...
  const handleShare = () => {
    if (navigator.share) {
      navigator.share({
        title: `FarmChain AI Provenance: ${batch.cropName}`,
        text: `Verified agricultural provenance certificate for ${batch.cropName} (${batch.id}).`,
          ...
          <div className="px-6 space-y-4">
            <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4">
              ...
              <div className="text-center sm:text-left space-y-1">
                ...
                <h1 className="text-2xl sm:text-3xl font-black text-white">{batch.cropName}</h1>
                <p className="text-sm text-slate-300">{batch.variety}</p>
                <div className="text-xs font-mono text-cyan-400 pt-1">
                  Batch Tag: <span className="font-bold">{batch.id}</span>
                </div>
              </div>
            </div>
          </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 text-center">
                <div className="p-2.5 rounded-xl bg-slate-900/80 border border-slate-800">
                  ...
                  <div className="text-lg font-black text-emerald-400 font-mono">
                    {batch.aiReport ? `${batch.aiReport.overallScore}/100` : '95/100'}
                  </div>
                </div>
                <div className="p-2.5 rounded-xl bg-slate-900/80 border border-slate-800">
                  <div className="text-[10px] text-slate-400">Awarded Grade</div>
                  <div className="text-lg font-black text-cyan-400 font-mono">
                    {batch.aiReport?.grade || 'Grade A+'}
                  </div>
                </div>
                <div className="p-2.5 rounded-xl bg-slate-900/80 border border-slate-800">
                  <div className="text-[10px] text-slate-400">Ripeness Index</div>
                  <div className="text-lg font-black text-amber-400 font-mono">
                    {batch.aiReport?.ripeness || 94}%
                  </div>
                </div>
                <div className="p-2.5 rounded-xl bg-slate-900/80 border border-slate-800">
                  <div className="text-[10px] text-slate-400">Color Uniformity</div>
                  <div className="text-lg font-black text-teal-400 font-mono">
                    {batch.aiReport?.colorUniformity || 97}%
                  </div>
                </div>
              </div>

              <div className="text-xs text-slate-400 flex items-center justify-between pt-1">
                <span>Pest / Fungal Defect Markers:</span>
                <span className="text-emerald-400 font-semibold font-mono">0.0% (Clean Pass)</span>
              </div>
            </div>
          </div>

          {/* Farm Origin & Harvest Bio */}
          <div className="px-6 space-y-3">
            <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400 flex items-center gap-1.5">
              <MapPin className="w-3.5 h-3.5 text-emerald-400" /> Origin Farm &amp; Grower
            </h3>
            <div className="p-4 rounded-2xl bg-slate-900/80 border border-slate-800 space-y-2 text-xs">
              <div className="flex justify-between">
                <span className="text-slate-400">Farm / Cooperative:</span>
                <span className="font-semibold text-white">{batch.farmName}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-400">Location:</span>
                <span className="text-slate-200">{batch.farmLocation}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-400">Harvest Date:</span>
                <span className="text-slate-200 font-mono">{batch.harvestDate}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-400">Geo-Coordinates:</span>
                <span className="text-cyan-400 font-mono">
                  {batch.farmCoordinates.lat}°N, {batch.farmCoordinates.lng}°W
                </span>
              </div>
            </div>
          </div>
              ...
              <div className="p-4 rounded-2xl bg-slate-900/80 border border-slate-800 space-y-3">
                <div className="flex items-center justify-between text-xs">
                  ...
                  <span className="font-bold text-emerald-400 font-mono">
                    {batch.iotTelemetry.currentTemp}°C (Within 2-5°C safe zone)
                  </span>
                </div>
                <div className="flex items-center justify-between text-xs">
                  <span className="text-slate-300">Relative Humidity:</span>
                  <span className="font-bold text-cyan-400 font-mono">
                    {batch.iotTelemetry.humidity}% Controlled
                  </span>
                </div>
            ...
            <div className="p-4 rounded-2xl bg-slate-950/90 border border-slate-800 space-y-2 text-[11px]">
              <div className="flex items-center justify-between text-slate-400">
                ...
                <span className="text-emerald-400 font-mono">
                  {batch.escrowStatus === 'Released to Farmer' ? 'Settled & Paid' : 'Escrow Protected'}
                </span>
              </div>
              <div className="font-mono text-slate-400 break-all bg-black/40 p-2 rounded">
                Contract: {batch.escrowContractAddress}
              </div>
            </div>
          </div>
        </div>
