...
export default function LogisticsPortal() {
  ...
  const handleUpdateCheckpoint = (e: React.FormEvent) => {
    ...
    setIsUpdating(true);
    setTimeout(() => {
      updateBatchStage(selectedBatch.id, newStage, checkpointLocation, checkpointNotes);
      setIsUpdating(false);
  ...
  const handleQuickAdvance = (batch: ProduceBatch) => {
    ...
    updateBatchStage(batch.id, nextStage, loc, note);
  };
